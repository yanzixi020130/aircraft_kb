...
_PROMPT_TEMPLATE = (
    ...
    "Task: estimate numeric values for the targets using the provided formula block fields and any known inputs.\n\n"
    "Rules:\n"
    "- Output ONLY a JSON object.\n"
    "- Keys MUST match the `key` field from the targets list.\n"
    "- Values MUST be JSON numbers (prefer) or null (only if truly impossible).\n"
    "- Do NOT include units or any extra text.\n"
    "- Use the provided unit as the reference unit if available.\n"
    "- Even if known_inputs are empty or insufficient, still provide a plausible numeric estimate based on the\n"
    "  formula_id/formula_name_zh/latex/expr/quantity_name_zh/target and typical domain values. Do NOT return null\n"
    "  just because inputs are missing.\n\n"
    "category: {category}\n"
    "extractid: {extractid}\n\n"
    "known_inputs (JSON):\n"
    "{known_inputs_json}\n\n"
