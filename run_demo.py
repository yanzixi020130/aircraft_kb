...
def main():
    category = "tilt_rotor"  # 换类别就改这�?
    print("=== Demo A: 单公式求�?(F_dynamic_pressure �?q) ===")
    ...
    print("\n=== Demo B: 前向链多步求�?rhoc,Vc,Swing,CL -> L (先解 q 再解 L) ===")
    ...
    print("\n=== 查询包含指定物理量的公式（不传 category/extractid�?===")
    res = find_formulas_by_quantity(
        quantity_id="q",
    )
    print(res)
    if res.get("status") == "ok":
        print(f"  找到 {len(res.get('formulas', []))} 个公式（跨所有类别）")

    print("\n=== 查询包含指定物理量的公式（只传 extractid�?===")
    res = find_formulas_by_quantity(
        extractid="performance",
        quantity_id="q",
    )
    print(res)
    if res.get("status") == "ok":
        print(f"  找到 {len(res.get('formulas', []))} 个公式（extractid='performance'）")

    print("\n=== 查询包含指定物理量的公式（传 category 和 extractid，有结果）===")
    res = find_formulas_by_quantity(
        category="tilt_rotor",
        extractid="Flight_Performance_Analysis_Extraction_Parameters",
        quantity_id="CL",
    )
    print(res)
    if res.get("status") == "ok":
        print(f"  找到 {len(res.get('formulas', []))} 个公式")
        for f in res.get("formulas", []):
            print(f"    - {f['formula_id']}: {f['formula_name_zh']}")

    print("\n=== 查询包含指定物理量的公式（不存在的 category�?===")
    res = find_formulas_by_quantity(
    ...
    print("\n=== 查询包含指定物理量的公式（不存在的 extractid�?===")
    res = find_formulas_by_quantity(
        category="tilt_rotor",
        extractid="nonexistent_extractid",
        quantity_id="q",
    )
    print(res)
